...
רשתות חברתיות הן כלי מרכזי להבנת דפוסי אינטראקציה בין אנשים, ובמיוחד בתעשיות יצירתיות כמו קולנוע. מחקר זה בוחן את **רשת שיתופי הפעולה של שחקני הקולנוע הישראלי** לאורך למעלה ממאה שנות קולנוע (1918-2025), תוך שימוש בכלי Machine Learning על גרפים.

### מטרות המחקר:
1. **איסוף וארגון נתונים**: בניית מאגר מקיף של סרטי הקולנוע הישראלי ושחקניהם מוויקיפדיה העברית
2. **ניתוח התפתחות הרשת**: מעקב אחר שינויים ברשת לאורך שלוש תקופות זמן (1918-1989, 1990-2009, 2010-2025)
...
- **תוצר**: [FILL: X] סרטים, [FILL: Y] שחקנים ייחודיים, טווח שנים 1918-2025
...
- **פיצול זמני**: שלוש תקופות (A: 1918-1989, B: 1990-2009, C: 2010-2025)
...
- `BeautifulSoup4` — פרסור HTML לטבלאות ותיבות מידע
...
**מדיניות נימוס:**
- User-Agent מותאם אישית: `"IsraeliActorsGraph/1.0 (course assignment)"`
- השהיה של 0.5 שניות בין בקשות
- התקדמות עם `tqdm` progress bar

**תוצאות:**
- **סה"כ סרטים**: [FILL: בדוק ב-`movies.csv` — len(df)]
- **טווח שנים**: 1918-2025 (107 שנים)
...
📁 **קוד**: `src/scraping.py` — פונקציות `fetch_list_page()`, `fetch_film_cast()`, `parse_cast_from_infobox()`
...
📁 **קוד**: `src/graph_build.py` — `build_collab_graph()`
...
|-----------|--------------|-----------|----------|
| A | 1918-1989 | ראשית הקולנוע הישראלי (71 שנים) | G_A |
| B | 1990-2009 | צמיחת הקולנוע (20 שנים) | G_B |
| C | 2010-2025 | העידן המודרני (15 שנים) | G_C |

כל גרף תקופה בנוי **רק מסרטים שיצאו בטווח השנים הרלוונטי**.

📁 **קוד**: `src/graph_build.py` — `split_by_periods()`
